...
kl_loss_labels = {key:0 for key in unique_labels_train}
...
# are from
def calculate_KLD_for_newdata(unique_labels, embeddings_train, embeddings_new, labels_list_train, labels_list_new, 
    ...
    for label in unique_labels:
        ...
        for i in range(embedding_label2.shape[0]):
            try:
...
kl_loss_new = calculate_KLD_for_newdata(unique_labels, embeddings_train, embeddings_new, labels_list_train, 
                                        labels_list_new, KLD_train, kl_loss_new)
number_of_diverted = {key:0 for key in unique_labels}
...
for label in kl_loss_new:
    number_of_diverted[unique_labels[label]] = len(kl_loss_new[label])
